...
class export_nextag {
	...
	public function __construct($directory) {
		...
		$this->export_path = $this->output_directory . '/' . 'nextag_'.Mage::app()->getStore()->getCode().'_'.$this->num_file.'.csv';
	...
	public function write_header() {
		$header_data = array();
		array_push($header_data,"Produktbezeichnung");
		array_push($header_data,"Produktbeschreibung");
		array_push($header_data,"Preis");
		array_push($header_data,"Klick-Out-URL");
		array_push($header_data,"Händler-Produktkat.");
		array_push($header_data,"EAN");
		array_push($header_data,"Hersteller-Art.-Nr.");
		array_push($header_data,"Hersteller");
		array_push($header_data,"ISBN");
		array_push($header_data,"Bild-URL");
		array_push($header_data,"Lagerstatus");
		array_push($header_data,"Produktzustand");
		array_push($header_data,"Ground Shipping");
		array_push($header_data,"Bild-URL-2");
		array_push($header_data,"Bild-URL-3");
		array_push($header_data,"Bild-URL-4");
		array_push($header_data,"Bild-URL-5");
	...
	public function write_row(&$prepared_product,&$product) {
		...
		if($prepared_product->out_of_stock == false){
			array_push($row_data,"Auf Lager");
		}else{
			array_push($row_data,"Nicht Auf Lager");	
		}

		array_push($row_data,"Neu"); // produktzustand
		array_push($row_data,"0,00");
		array_push($row_data,""); // Bild-URL-2 ...
		array_push($row_data,"");
		array_push($row_data,"");
		array_push($row_data,"");
